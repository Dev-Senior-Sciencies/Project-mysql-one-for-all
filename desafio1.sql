...
CREATE TABLE t_planing(
    planing_id INT NOT NULL AUTO_INCREMENT,
    planing VARCHAR(50) NOT NULL,
    planing_value DECIMAL(4, 2) NOT NULL,
    CONSTRAINT PRIMARY KEY(planing_id)
) engine = InnoDB;

INSERT INTO t_planing(planing, planing_value)
VALUES
  ('gratuito', 0.00),
  ('universitário', 5.99),
  ('pessoal', 6.99),
...
CREATE TABLE t_user(
    CONSTRAINT PRIMARY KEY(use_id),
    use_id INT NOT NULL AUTO_INCREMENT,
    usuario VARCHAR(50) NOT NULL,
    age_user INT NOT NULL,
    planing_id INT NOT NULL,
    date_ass DATE NOT NULL,
    FOREIGN KEY(planing_id) REFERENCES t_planing(planing_id)
) engine = InnoDB;

INSERT INTO t_user(usuario, age_user, planing_id, date_ass)
VALUES
...
CREATE TABLE t_favorite(
    use_id INT,
    favorite_id INT,
    CONSTRAINT PRIMARY KEY (use_id, favorite_id),
        FOREIGN KEY (use_id) REFERENCES t_user (use_id),
        FOREIGN KEY (favorite_id) REFERENCES t_artistic (favorite_id)
);

INSERT INTO t_favorite(use_id, favorite_id)
VALUES
  (1, 1),
  (1, 4),
  (1, 3),
...
CREATE TABLE t_album(
    ...
    age_licen INT NOT NULL,
    FOREIGN KEY (favorite_id) REFERENCES t_artistic (favorite_id)
) engine = InnoDB;

INSERT INTO t_album(album, favorite_id, age_licen)
VALUES
  ('Envious', 1, 1990),
  ('Exuberant', 1, 1993),
...
CREATE TABLE t_songs(
    ...
    duract_seconds INT NOT NULL,
      FOREIGN KEY (favorite_id) REFERENCES t_artistic (favorite_id),
      FOREIGN KEY (album_id) REFERENCES t_album (album_id)
) engine = InnoDB;

INSERT INTO t_songs(songs, favorite_id, album_id, duract_seconds)
VALUES
  ('Soul For Us', 1, 1, 200),
...
CREATE TABLE t_history_date (
	use_id INT NOT NULL,
  songs_id INT NOT NULL,
  history TIMESTAMP NOT NULL,
  CONSTRAINT hitoric_id PRIMARY KEY (use_id, songs_id),
  FOREIGN KEY (use_id) REFERENCES t_user(use_id),
  FOREIGN KEY (songs_id) REFERENCES t_songs(songs_id)
) engine = InnoDB;

INSERT INTO t_history_date(use_id, songs_id, history)
VALUES
